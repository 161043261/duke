...
function findAnagrams1(s, p) {
    ...
    for (; r < p.length; r++) {
        p2num.set(p[r], ((_a = p2num.get(p[r])) !== null && _a !== void 0 ? _a : 0) /* 加括号 */ + 1);
        s2num.set(s[r], ((_b = s2num.get(s[r])) !== null && _b !== void 0 ? _b : 0) /* 加括号 */ + 1);
    }
    // r == p.length
    const check = () => {
        for (let key of p2num.keys()) {
    ...
    for (; r <= s.length; l++, r++) {
        ...
        s2num.set(s[l], s2num.get(s[l]) - 1);
        s2num.set(s[r], ((_c = s2num.get(s[r])) !== null && _c !== void 0 ? _c : 0) /* 加括号 */ + 1);
    }
    return ans;
}
