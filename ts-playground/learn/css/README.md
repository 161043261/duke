...
1. 对未浮动的兄弟元素的影响
1. 前面的未浮动的兄弟元素: 无影响 (参考 "独占一行")
1. 后面的未浮动的兄弟元素, 可以占据浮动元素未浮动时的位置 (参考 "未浮动")
1. 对父元素的影响
1. 浮动元素不能撑开父元素的高度, 父元素高度塌陷
1. 父元素的宽度仍然限制浮动元素
...
4. 在所有浮动元素后面, 添加一个块级元素, 该块级元素设置 `clear: both`
5. 使用伪元素: 在父元素后创建一个子元素
...
布局原则: 设置浮动时, 兄弟元素要么全部都浮动, 要么全部都不浮动
